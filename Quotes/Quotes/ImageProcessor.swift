...
class ImageProcessor {
  ...
  func process(in imageView: UIImageView, singleWords: Bool, callback: @escaping (_ text: String, _ scaledElements: [ScaledElement], _ textElement: [TextElement]) -> Void) {
...
//      print(result.blocks.count)
        ...
        for block in result.blocks {
          for line in block.lines {
            for element in line.elements {
              ...
              let scaledElement = ScaledElement(frame: frame, shapeLayer: shapeLayer, selectedShapeLayer: selectedShapeLayer)
              scaledElements.append(scaledElement)
              textElements.append(TextElement(text: element.text, scaledElement: scaledElement, index: index))
              index += 1
            }
          }
        }
      ...
      callback(result.text, scaledElements, textElements)
    }
  }
